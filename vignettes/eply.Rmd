...
# Functions `veval()` and `seval()`

`eply()` uses `veval()` to evaluate each row of `.expr`. Function `.seval()` simplifies the result.

```{r}
veval(c("subset(mtcars, cyl == 4, mpg, TRUE)", "mtcars[1:2, 1:2]"))
veval(c("a+1", "b+2"), .with)
seval(c("a+1", "b+2"), .with)
```
